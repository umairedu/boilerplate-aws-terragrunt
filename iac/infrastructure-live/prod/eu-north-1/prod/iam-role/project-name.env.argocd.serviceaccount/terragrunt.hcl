...
dependencies {
  paths = ["../../iam-policy/YOUR-PROJECT-NAME.env.argocd.serviceaccount",
  "../../eks/YOUR-PROJECT-NAME.env.eks"]
}

dependency "argocd" {
  config_path = "../../iam-policy/YOUR-PROJECT-NAME.env.argocd.serviceaccount"
  mock_outputs = {
...
inputs = {
  ...
  oidc_fully_qualified_subjects = ["system:serviceaccount:argocd:${include.root.locals.environment_vars.locals.project}-${include.root.locals.env}-argocd-repo-server"]
